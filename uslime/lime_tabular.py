...
from lime_codes.discretize import QuartileDiscretizer
from lime_codes.discretize import DecileDiscretizer
from lime_codes.discretize import EntropyDiscretizer
from lime_codes.discretize import BaseDiscretizer
from lime_codes.discretize import StatsDiscretizer
...
class LimeTabularExplainer(object):
    ...
    def __init__(self,
        ...
        self.generation_coef = generation_coef
        self.kernel_width_coef = kernel_width_coef
        if kernel_width is None:
            kernel_width = np.sqrt(training_data.shape[1]) * self.kernel_width_coef
    ...
    def data_inverse(self,
        ...
        Args:
            ...
            desired_variance: only for gaussian, the variance of the normal
        ...
        if self.discretizer is None:
            ...
            if sampling_method == 'gaussian':
                data = self.random_state.normal(0, desired_variance, num_samples * num_cols
...
class USLimeTabularExplainer(LimeTabularExplainer):
    ...
    def __init__(self,
        ...
        super(USLimeTabularExplainer, self).__init__(training_data, mode, training_labels, feature_names,
                                                      ...
                                                      random_state, training_data_stats, kernel_width_coef, generation_coef)
    ...
    def explain_instance(self,
        ...
        data, inverse = self.data_inverse(data_row, self.generation_coef*num_samples,
                                          sampling_method, desired_variance=.5)
        # -----------------------------
        ...
        target_instance_label = int(target_instance_proba[1])
        ...
        kernel_width = np.sqrt(scaled_data.shape[1]) * self.kernel_width_coef
        ...
        distance_metric = "euclidean"
        distance_to_target = sklearn.metrics.pairwise_distances(new_data,
        ...
        filtered_yss[0] = target_instance_proba

        # calculating our weights
        distances_ex = np.array([np.abs(0.5-y) for y in filtered_yss[:, target_instance_label]])

        # test the lime distances
        distance_metric = "euclidean"
        distances = sklearn.metrics.pairwise_distances(last_filtered_data,
                                        data_row.reshape(1, -1),
                                        metric=distance_metric
                                        ).ravel()
